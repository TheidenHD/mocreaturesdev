...
public class MoCEntityShark extends MoCEntityTameableAquatic {
    ...
    public MobEntity FindTarget(Entity entity, double d) {
        double d1 = -1D;
        MobEntity entityliving = null;
        List<Entity> list = this.world.getEntitiesWithinAABBExcludingEntity(this, getBoundingBox().grow(d));
        for (Entity o : list) {
            if (!(o instanceof MobEntity) || (o instanceof MoCEntityAquatic) || (o instanceof MoCEntityEgg) || (o instanceof PlayerEntity) || ((o instanceof EntityWolf) && !(MoCreatures.proxy.attackWolves)) || ((o instanceof MoCEntityHorse) && !(MoCreatures.proxy.attackHorses))) {
                continue;
            } else {
            ...
            if (((d < 0.0D) || (d2 < (d * d))) && ((d1 == -1D) || (d2 < d1)) && ((MobEntity) o).canEntityBeSeen(entity)) {
                d1 = d2;
                entityliving = (MobEntity) o;
            }
        }
        return entityliving;
    }
